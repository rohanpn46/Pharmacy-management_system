...
def admin():
    ...
    ## CUSTOMERS
        ...
        order_menu = ["View", "Delete"]
        order_choice = st.sidebar.selectbox("Order Actions", order_menu)

        # Set session state flag for order deletion tracking
        if 'order_deleted' not in st.session_state:
            st.session_state.order_deleted = False

        if order_choice == "View":
            st.subheader("Order Details")
            # If an order was recently deleted, refresh data
            if st.session_state.order_deleted:
                order_result = order_view_all_data()  # Re-fetch the latest order data
                st.session_state.order_deleted = False  # Reset the deletion flag
            else:
                order_result = order_view_all_data()  # Load order data if no recent deletion
        ...
        elif order_choice == "Delete":
            st.subheader("Delete an Order")
            order_id = st.text_input("Enter Order ID to delete")
            if st.button("Delete Order"):
                order_delete(order_id)
                st.success(f"Order with ID '{order_id}' has been deleted successfully.")
                
                # Set the deletion flag to true to refresh "View" on next load
                st.session_state.order_deleted = True

    elif choice == "About":
        st.subheader("DBMS Mini Project")
        st.subheader("By")
        st.subheader("Nischith S (259)")
        st.subheader("Suhas B M (221)")
        st.subheader("Suhas B H (220)")
        st.subheader("Rohan P N (257)")
        st.subheader("Vishwas H T (248)")
        st.subheader("Thanman Mahesh (236)")
